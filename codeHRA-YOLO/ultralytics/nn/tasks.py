...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Detect_DyHead,
                          Detect_Efficient, DetectAux, Segment, Segment_Efficient)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, Detect_DyHead,
                          Detect_Efficient, DetectAux, Segment, Segment_Efficient, Pose)):
            s = 640  # 2x min stride
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        dec_bboxes = torch.cat([enc_bboxes.unsqueeze(0), dec_bboxes])  # (hra, bs, 300, 4)
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Detect_DyHead,  Detect_Efficient, DetectAux,  Segment, Segment_Efficient):
            m.inplace = inplace
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, CBF,CBM,ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3,  C3_DWR, C2f_DWR):
        ...
        elif m in (Detect, Detect_DyHead,
                   Detect_Efficient, DetectAux,  Segment, Segment_Efficient, Pose):
...
def guess_model_task(model):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Detect, Detect_DyHead, Detect_Efficient, DetectAux,
                             )):
                return 'detect'
